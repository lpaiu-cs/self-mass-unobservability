# Failure Ledger (Dynamic-Chi Track)

Status: Note. This is the dynamic-chi track's early working ledger, preserved under its own name when the branch merged with the theorem track (whose boundary-risk register owns `docs/failure-ledger.md`). The authoritative dynamic-track record is the `notes/REQUEST10_*` chain and `request10_external/`.
