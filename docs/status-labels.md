...
| `Assumption` | A posited premise that defines the theorem domain. Neither derived nor open -- the positive theorem is *conditional* on it. Used for `A1`-`A8` (see [`assumptions-ledger.md`](assumptions-ledger.md)). | "There is no orbital-timescale internal state variable (`A4`)." |
...
- `A1`-`A8`: `Conjectural` -> `Assumption` (they are premises).
