...
| A2 | Conjectural | The M2 sector is nearly spherical, nonspinning, and parity-even. | Keeps the operator search space inside the minimal free-fall sector requested for this theorem candidate. | Spin, parity-odd structures, or large asphericity add new operator families at the same order. |
...
## Non-Assumption For M2

- Status: Proven. Finite invariant basis closure is not listed as an assumption in M2.
- Status: Conjectural. Basis closure is now the explicit burden of [`docs/theorem-A-freefall.md`](theorem-A-freefall.md) and [`../lemmas/05-finite-basis-closure.md`](../lemmas/05-finite-basis-closure.md).
