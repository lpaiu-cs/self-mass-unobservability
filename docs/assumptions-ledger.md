...
- Status: Counterexample candidate. The nonlinear second-order branch keeps A4 violated and adds either nonlinear internal drive `beta_F2 F^2` or nonlinear readout coefficients `lambda_Fchi,lambda_chi2`; those coefficients are shared nuisance parameters, not free per-frequency knobs.
- Status: Proven. Nonlinear second-order sideband existence alone does not count; the active target is a shared quadratic denominator across linear and generated lines after finite static nonlinear comparator budgets are exceeded.
- Status: Conjectural. The nonlinear second-order detectability audit assumes the generated nonlinear component can be separated from the linear exact-in-e harmonic at the same frequency; without this component separation, generated-line detectability collapses to static nonlinear mimicry.
