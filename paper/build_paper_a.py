...
DEFAULT_SOURCE = ROOT / "paper-A-collapse-theorem.md"
DEFAULT_OUTPUT = ROOT / "paper-A-collapse-theorem.tex"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Paper A LaTeX draft.")
...
def parse_document(text: str) -> tuple[str, dict[str, str], str, list[str]]:
    ...
    if not lines or not lines[0].startswith("# "):
        raise ValueError("Paper A source must start with a level-1 title")
    ...
    if i >= len(lines) or lines[i].strip() != "## Abstract":
        raise ValueError("Paper A source must contain a '## Abstract' section")
    i += 1
...
def build_tex(title: str, metadata: dict[str, str], abstract: str, body: list[str], source: Path) -> str:
    ...
    return f"""% Auto-generated by paper/build_paper_a.py from {source.name}.
