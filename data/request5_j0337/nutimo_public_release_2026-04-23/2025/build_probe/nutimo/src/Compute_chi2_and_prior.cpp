...
int main ( int argc, char *argv[] ) {
  ...
  fonction.Reconstruct(argv[1], argv[2]);
 
  ndim = fonction.Get_nfitted_params();   // set ndima

  double walker1d[ndim];
  
  for (int i = 0; i < ndim ; i++) walker1d[i] = 0.;
  
//   Load_chain(argv[3], chain, ndim, nb_walkers, chain_size, chain_freq);
  //Init_from_prev_chain(argv[3], walker1d, ndim, nb_walkers);
  
  double totlnposterior = fonction(walker1d);
  
  printf("\n totlnposterior lnpost prior : %.10e %.10e %.10e \n\n", totlnposterior, fonction.lastlnlikelyhood, fonction.lastprior);
    
//   for (int k = 0 ; k < chain_size ; k++) free(chain[k]);
//   free(chain);
