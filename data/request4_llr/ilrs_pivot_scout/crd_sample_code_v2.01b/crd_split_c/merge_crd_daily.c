...
**   20 Apr 2018 - Update for CRD v2.00. rlr.
**
**-----------------------------------------------------------------------*/
int
main ()
{
  ...
  for (item = readdir (dirp); item != NULL; item = readdir (dirp))
	...
	continue;
	  ...
	  strncmp (&item->d_name[strlen(item->d_name)-4], ".NPT", 4) != 0)
	continue;
	...
	continue;		/* silently ignore the rest */
	  ...
	  testname1[strlen(testname)]= '\0';
		...
		&station_id,satname,crd_lit,&dt,&hr,&rel);
          ...
          sprintf (passname[(nitems)++], "%8ld %02d %s %04d %s", dt,
	  ...
	  getifield(testname, 11, 4, &hhmm);
	    ...
	    getifield (testname, 16, 3, &sic);
          ...
          sprintf (passname[(nitems)++], "%4d%02d%02d %02d %s %4d %s", 
  ...
  for (i = 0; i < nitems; i++)
    {
      ...
      sscanf (passname[i], "%*d %d %s %*d %s",&hour,satname,satpassname);
	     ...
	     char *target, int *delta)
{
  ...
  int tcospar_id, tsic, tnorad_id = 0;
  char *status;
  int i, l;
