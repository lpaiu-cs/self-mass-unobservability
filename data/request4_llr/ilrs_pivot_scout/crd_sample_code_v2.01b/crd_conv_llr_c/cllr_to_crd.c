...
void read_llr0 (char *, struct rllr0 *);
void read_llr1 (char *, struct rllr1 *);
void read_llr2 (char *, struct rllr2 *);
void read_llr3 (char *, struct rllr3 *);
void read_llr4 (char *, struct rllr4 *);
void read_llr5 (char *, struct rllr5 *);
void read_llr6 (char *, struct rllr6 *);
void read_llr7 (char *, struct rllr7 *);
void grtojd (int, int, int, int, int, double, double *, double *);
 ...
 *   		   1.01b. rlr.
 *   Sep 13, 2018 -Changes for version 2 of CRD format. rlr.
 *   Aug 23, 2019 -Changes for version 2 NA fields. rlr.
 *
**-----------------------------------------------------------------------*/
int
main (argc, argv)
...
/*  Greetings */
  printf("Convert COSPAR Lunar Normalpoints to CRD Format: version 2.00 09/14/2018\n");
...
/* Station header */
  ...
  //h2.cdp_sys_num = -1;
  //h2.cdp_occ_num = -1;
  h2.cdp_sys_num = NA_VALUE;	// 082319 NA
  h2.cdp_occ_num = NA_VALUE;	// 082319 NA
...
/* Target header */
  ...
  h3.target_class = 1;		/* lunar */
  h3.target_loc = 3;		/* lunar */
...
/* Calibration record */
  ...
  **/
  ...
  if (llr1.observatory_code == 71110)	/* Mcd 2.7m */
    ...
    {
      d40.cal_shift_type_ind = NA_VALUE;	// 082319 NA
      d40.cal_type_ind = NA_VALUE;	// 082319 NA
    }
...
/* Range Record */
  if (recd_type == 3)
	...
	llr3.hour * 3600 + llr3.min * 60 + (llr3.sec+ llr2.clock_offset)/ 
      ...
      d10.stop_number = 0;
      d10.xcv_amp = NA_VALUE;	// 082319 NA
      d10.xmt_amp = NA_VALUE;	// 082319 NA
  ...
  /* Meteorology Record */
      ...
      strcpy (d21.weather_conditions, "na");
      d21.visibility= NA_VALUE;	// 082319 NA
      d21.sky_clarity= NA_VALUEF;	// 082319 NA
      d21.atmospheric_seeing= llr2.seeing/10.;
      d21.cloud_cover= NA_VALUE;	// 082319 NA
    }
/* Normalpoint Record */
  else if (recd_type == 4)
    {
...
/* Range Supplement Record */
  ...
  d12.range_rate = NA_VALUEF;	// 101519 NA
}
