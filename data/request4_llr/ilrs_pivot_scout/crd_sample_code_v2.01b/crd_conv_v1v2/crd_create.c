 ...
 * Also creates additional new records and fields for version 2.00
 *
 ...
 *
 * Calling sequence:
 *   crd_create crd_input crd_output [info_override_file]
 *
 * Author: Randall Ricklefs / Univ of Texas / Center for Space Research
 *
 * History:
 *   July 1, 2018 - Initial version.
 *   October 16, 2019 - V 2.01 with NA, C7/C42, etc. 
 *
**-----------------------------------------------------------------------*/
...
{
  char str[512];
  FILE *str_in, *str_out, *str_add;
  int i;

  // read from override file:
  int pad_override= 0;
  int pad_search= 0;
  int cal_span= 0, trans_amp= 0;
  int amp_in_use;
  double cal_return_rate= 0.0, range_rate= 0.0, az_rate= 0.0, el_rate= 0.0;
  double sky_temp= 0.0;
  double snr= 0.0;
  double amp_gain, amp_bandwidth;
  char network[11]= "";	// WATCH OUT! Not length checking on read!
  ...
  if (argc == 3)
  ...
  else
    {
      ...
      printf("Usage: input_CRD output_CRD [fill_in_file]\n");
      exit (-1);
    }
  ...
  // Open and read the fill in file
  if (argc == 4)
    {
      if ((str_add = fopen (argv[3], "r")) == NULL)
        {
          printf ("Could not open file %s\n", argv[3]);
          exit (1);
        }

      while (fgets (str, 512, str_add) != NULL)
        {
          if (strncmp (str, "pad search", 10) == 0)
            {
              sscanf (&str[10], "%d", &pad_search);
            }
          else if (strncmp (str, "pad override", 12) == 0)
            {
              sscanf (&str[12], "%d", &pad_override);
            }
          else if (strncmp (str, "cal return rate", 15) == 0)
            {
              sscanf (&str[15], "%lf", &cal_return_rate);
            }
          else if (strncmp (str, "cal span", 8) == 0)
            {
              sscanf (&str[8], "%d", &cal_span);
            }
          else if (strncmp (str, "range rate", 10) == 0)
            {
              sscanf (&str[10], "%lf", &range_rate);
            }
          else if (strncmp (str, "az rate", 7) == 0)
            {
              sscanf (&str[7], "%lf", &az_rate);
            }
          else if (strncmp (str, "el rate", 7) == 0)
            {
              sscanf (&str[7], "%lf", &el_rate);
            }
          else if (strncmp (str, "trans amp", 9) == 0)
            {
              sscanf (&str[9], "%d", &trans_amp);
            }
          else if (strncmp (str, "snr", 3) == 0)
            {
              sscanf (&str[3], "%lf", &snr);
            }
          else if (strncmp (str, "sky temp", 8) == 0)
            {
              sscanf (&str[8], "%lf", &sky_temp);
            }
          else if (strncmp (str, "network", 7) == 0)
            {
              sscanf (&str[7], "%s", network);
            }
          else if (strncmp (str, "amp gain", 8) == 0)
            {
              sscanf (&str[8], "%lf", &amp_gain);
            }
          else if (strncmp (str, "amp bandwidth", 13) == 0)
            {
              sscanf (&str[13], "%lf", &amp_bandwidth);
            }
          else if (strncmp (str, "amp in use", 10) == 0)
            {
              sscanf (&str[10], "%d", &amp_in_use);
            }
  ...
  while (fgets (str, 512, str_in) != NULL)
    {
      // Add h5 for version 2
      if ((str[0] == 'C' || str[0] == 'c' || 
          (isdigit (str[0]) && str[0] != '0')) && 
      //if ((str[0] == 'C' || str[0] == 'c' || isdigit (str[0])) && 
               h1.format_version == 2 && noh5)
        {
//printf ("h5: pt %d yoc %d dnt %s pp %s seq %d\n", h5.prediction_type,
//h5.year_of_century, h5.date_and_time, h5.prediction_provider, 
//h5.sequence_number);
          h5.year_of_century= h4.start_year%100;
          sprintf (h5.date_and_time, "%02d%02d%02d", 
                   h4.start_mon, h4.start_day, h4.start_hour);
          grtodoy (h5.year_of_century, h4.start_mon, h4.start_day,
                   &h5.sequence_number); 
          h5.sequence_number*= 100; 
          h5.sequence_number+= 2; 
          write_h5 (str_out, h5);
          noh5= 0;
        }
      // Add c5 and c6 for version 2
      //if (isdigit (str[0]) && h1.format_version == 2 && noc56)
      if ((isdigit (str[0]) && str[0] != '0') && 
          h1.format_version == 2 && noc56)
        {
          write_c5 (str_out, c5);
          write_c6 (str_out, c6);
          noc56= 0;
	  ...
	  //write_h1 (str_out, h1);
        ...
        {
          read_h2 (str, &h2);
          if (pad_search != 0 && h2.cdp_pad_id != pad_search)
            {
              // Not a match -- go to the end of pass.
              while (fgets (str, 512, str_in) != NULL)
                {
	  ...
	  write_h8 (str_out);
          ...
          // Start over with H5 and C5/c6 search
          noh5= 1;
          noc56= 1;
        }
        ...
        {
          read_c0 (str, &c0);
          // Add new config records for v2 (if there is room)
          if (h1.format_version == 2)
          for (i=1; i<8; i++)
            {
              if (c0.config_ids[i][0] != '\0') continue;
                strcpy (c0.config_ids[i+1], "pgms");
                strcpy (c0.config_ids[i+2], "mets");
                break;
            }
          write_c0 (str_out, c0);
        }
	  ...
	  if (h1.format_version == 2)
        ...
        {
          read_c3 (str, &c3);
	  if (h1.format_version == 2 && c3.epoch_delay_corr < 0) 
	  ...
	  if (h1.format_version == 2)
          ...
          write_c4 (str_out, c4);
               ...
               strncmp (str, "c5", 2) == 0)
        {
          // C5 doesn't exist in V1, so drop them
        }
      else if (strncmp (str, "C6", 2) == 0 ||
               strncmp (str, "c6", 2) == 0)
        {
	  ...
	  if (h1.format_version == 2)
      ...
      else if (strncmp (str, "30", 2) == 0)
        {
          ...
          if (h1.format_version == 2) 
            {
              d30.azimuth_rate= az_rate;
              d30.elevation_rate= el_rate;
        ...
        {
          read_40 (str, &d40);
          //printf ("cs %d crr %f\n", cal_span, cal_return_rate);
          if (h1.format_version == 2) 
            {
              d40.cal_span= cal_span;
              d40.cal_return_rate= cal_return_rate;
          ...
          if (h1.format_version == 2 && d40.cal_span == 3)
            {
              d41= d40;
              d41.cal_span= 1;
              d41.sec_of_day= d40.sec_of_day- 3600;
              if (d41.sec_of_day < 0) d41.sec_of_day+= 86400.0;
              if (d40.num_points_recorded != NA_VALUE)	// 082319 - NA
                {
                  d41.num_points_recorded= d40.num_points_recorded/3.0; 
                  d41.num_points_used= d40.num_points_used*2.0/3.0; 
                }
              write_41 (str_out, d41);
              d41.cal_span= 2;
              d41.sec_of_day= d40.sec_of_day+ 3600;
              if (d41.sec_of_day > 86400.0) d41.sec_of_day-= 86400.0;
              if (d40.num_points_recorded != NA_VALUE)	// 082319 - NA
                {
                  d41.num_points_recorded= 
                    d40.num_points_recorded- d41.num_points_recorded;
                  d41.num_points_used= 
                    d40.num_points_used- d41.num_points_used;
                }
              write_41 (str_out, d41);
            }
        }
