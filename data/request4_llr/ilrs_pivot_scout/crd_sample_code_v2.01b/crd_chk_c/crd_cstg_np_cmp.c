...
void read_cstg_hdr (char *, struct cstg_hdr *) ;
void read_cstg_np (char *, struct cstg_np *);
void read_cstg_sampled_eng (char *, struct cstg_sed *);
	  ...
	  if (!read_sep)
	      ...
	      do
		{
		  if ((status = fgets (str, 256, str_in2)) != NULL &&
		    ...
		    d11_missing++;
		  if (debug)
		    printf ("np [%s] status %s d11_missing %d\n", str,
			    status, d11_missing);
		}
