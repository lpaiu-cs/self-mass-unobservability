...
void write_merit_fr (FILE *, struct merit_fr);
 ...
 *   Apr 20, 2018 - Included code to read CRD versions 1 and 2. rlr.
 *   Sep 18, 2018 - clean up. rlr.
 *   Dec 23, 2020 - Adjusted round off handling for az/el and pth. rlr.
 *   Dec 23, 2020 - Adde Jose Rodríguez' code to fix "new day" issue. rlr.
 *
**-----------------------------------------------------------------------*/
...
/*  Greetings */
  printf("Convert CRD Fullrate to Merit II Format: version 2.01b 12/23/2028\n");
...
/* read and process records critical to header... */
      ...
      while ((status = fgets (str, 256, str_in)) != NULL)
	{
	  ...
	  if (strncmp (str, "h1", 2) == 0 || 
	    ...
	    {
	      read_h1 (str, &h1);
	      fr.format_version= 3;	// Nerit format version
	    }
	  ...
	  else if (strncmp (str, "20", 2) == 0)
	    {
	      ...
	      **/
	      if (!disna(d20.humidity))
                {
                  save_humidity[n20h] = d20.humidity;
                  if (n20h < 499) n20h++;     /* Should scream, but >500 unlikely */
                }
              if (!disna(d20.pressure))
                {
                  save_pressure[n20p] = d20.pressure * 10;
                  if (n20p < 499) n20p++;     /* Should scream, but >500 unlikely */
                }
              if (!disna(d20.temperature))
                {
                  save_temperature[n20t] = d20.temperature * 10;
                  if (n20t < 499) n20t++;     /* Should scream, but >500 unlikely */
                }
	      //printf ("h %f sh %f p %f sp %f t %f st %f\n", d20.humidity, save_humidity[n20], d20.pressure, save_pressure[n20], d20.temperature, save_temperature[n20]);
	  ...
	  else if (strncmp (str, "30", 2) == 0)
	    {
		  ...
		  **/
                  if (!disna(d30.azimuth))
                    {
                      save_azimuth[n30a] = d30.azimuth * 1.e4;
                      if (n30a < 499) n30a++;     /* Should scream, but >500 unlikely */
                    }
                  if (!disna(d30.elevation))
                    {
                      save_elevation[n30e] = d30.elevation * 1.e4;
                      if (n30e < 499) n30e++;     /* Should scream, but >500 unlikely */
                    }

		  //printf ("az %f saz %f el %f sel %f\n", d30.azimuth,  save_azimuth[n30], d30.elevation, save_elevation[n30]);
	  ...
	  if (strncmp (str, "h8", 2) == 0 ||
	      ...
	      if (fr_ready)
		{
	      ...
	      if (d10.sec_of_day < old_d10sec_of_day ||
                  ...
                  (first_10 && header_sod > d10.sec_of_day + 1)) // +1 because of rounding
                {
		  fr.doy++;
                  fnewday= 1;
                  if (fr.doy > final_doy)
	      ...
	      fr.humidity = interp((double)d10.sec_of_day+fnewday*86400.e0,
                 save_met_sod,save_humidity,n20); // + 0.499999;
	      fr.pressure = interp((double)d10.sec_of_day+fnewday*86400.e0,
                 save_met_sod,save_pressure,n20); // + 0.499999;
	      fr.temperature = interp((double)d10.sec_of_day+fnewday*86400.e0,
                 save_met_sod,save_temperature,n20); // + 0.499999;
	      ...
	      fr.azimuth = interp((double)d10.sec_of_day+fnewday*86400.e0,
                 save_ang_sod,save_azimuth,n30);//+ 0.499999;
	      fr.elevation = interp((double)d10.sec_of_day+fnewday*86400.e0,
                 save_ang_sod,save_elevation,n30);//+ 0.499999;
	      //printf ("iaz %f iel %f\n", fr.azimuth, fr.elevation);
	  ...
	  /* npts not likely, but just in case... */
	      ...
	      if (fr_ready)
		{
	      ...
	      d1xsec_of_day = d11.sec_of_day;
              ...
              /* New day ...  since last '11' or header; don't rely on 
               * stop time as it is '0' for some stations */
	      if (d11.sec_of_day < old_d11sec_of_day ||
                  //(first_11 && header_sod > d11.sec_of_day))
		  (first_11 && header_sod > d11.sec_of_day + 1)) // +1 because of rounding
                {
		  fr.doy++;
                  fnewday= 1;
                  if (fr.doy > final_doy)
