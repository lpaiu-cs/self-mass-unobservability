...
void write_cstg_sed (FILE *, struct cstg_sed);
void write_cstg_hdr (FILE *, struct cstg_hdr, int);
 ...
 *   May  6, 2019 - Set SCH and SCI to zero as default, because CRD has
 *                  dropped the "60" compatibility record. rlr.
 *
**-----------------------------------------------------------------------*/
...
/*  Greetings */
  printf("Convert CRD Quicklook to CSTG Format: version 2.00 05/06/2019\n");
...
/* Initialize variables for each pass */
      ...
      old_d10sec_of_day=-1;
      hdr.sys_change_ind = 0;   /* 05/06/19 - not in CRD V2 */
      hdr.sys_config_ind = 0;   /* 05/06/19 - not in CRD V2 */
