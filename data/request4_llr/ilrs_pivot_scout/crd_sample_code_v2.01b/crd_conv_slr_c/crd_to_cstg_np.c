...
void write_cstg_np (FILE *, struct cstg_np);
...
/*  Greetings */
  printf("Convert CRD Normalpoints to CSTG Format: version 2.00 05/06/2019\n");
      ...
      while ((status = fgets (str, 256, str_in)) != NULL)
	{
	  if (strncmp (str, "h1", 2) == 0 ||
	  ...
	  else if (strncmp (str, "11", 2) == 0)
	    {
	      ...
	      // Take into account whether the values are "na" value
	      ...
	      if (np.time_of_flight >= 1.e0)
		{
	      ...
	      np.num_ranges = d11.num_ranges;
              ...
              np.humidity = interp((double)d11.sec_of_day+fnewday*86400.e0,
                 save_met_sod,save_humidity,n20h); // + 0.499999; //0.51;
              np.pressure = interp((double)d11.sec_of_day+fnewday*86400.e0,
                 save_met_sod,save_pressure,n20p); // + 0.499999; //0.51;
              np.temperature = interp((double)d11.sec_of_day+fnewday*86400.e0,
                 save_met_sod,save_temperature,n20t); // + 0.499999; //0.51;
