...
C     Keep ephemeris library quiet inside the MLRS batch workflow.
C     write(6,*)
C     Suppress informational banner so MLRS .errorc remains a failure channel.
C     The original MLRS workflow expects a quiet ephemeris reader here.
C     write(6,'(a27,i3.3)')' JPL planetary ephemeris DE',NUMDE
C     write(6,*)'Requested output units are :'
C     if(AU_KM)then
C       if(IAU_AU)then
C         write(6,*)'IAU au for distance'
C         if(DAY_SEC)then
C           write(6,*)'IAU au/day for velocity'
C         else
C           write(6,*)'IAU au/sec for velocity'
C         endif
C       else
C         write(6,'(a2,i3.3,a16)')'DE',NUMDE,' au for distance'
C         if(DAY_SEC)then
C           write(6,'(a2,i3.3,a20)')'DE',NUMDE,' au/day for velocity'
C         else
C           write(6,'(a2,i3.3,a20)')'DE',NUMDE,' au/sec for velocity'
C         endif
C       endif
C     else
C       write(6,*)'km for distance'
C       if(DAY_SEC)then
C         write(6,*)'km/day for velocity'
C       else
C         write(6,*)'km/sec for velocity'
C       endif
C     endif
