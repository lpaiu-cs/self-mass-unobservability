...
- Status: Proven. M4 resolved that omission by explicit contraction enumeration and, under its generic gradient-block model, promoted `divE2` and `mixedGradE2` to survivors (five -> seven).
- Status: Proven. The M4 promotion was itself later found defective (external adversarial review, 2026-07-12): the generic model dropped the Schwarz total symmetry of `\nabla_k E_{ij} = \partial_k\partial_i\partial_j\Phi` and the vacuum trace condition already imposed on `E` itself. With the correct STF-3 kinematics, `divE2 = 0` and `mixedGradE2 = gradE2`, and the survivor set returns to this note's five-element target — now with *both* catalog exhaustiveness and the correct kinematics in force.
- Status: Note. The pre-M4 five-element target was therefore right for an incomplete reason: this note lacked the enumeration needed to certify it, and M4's enumeration lacked the kinematics needed to count correctly. The corrected contraction-level statement lives in [`07-gradient-sector-audit.md`](07-gradient-sector-audit.md), [`08-mixed-time-derivative-audit.md`](08-mixed-time-derivative-audit.md), and [`09-survivor-independence-delta4.md`](09-survivor-independence-delta4.md).
